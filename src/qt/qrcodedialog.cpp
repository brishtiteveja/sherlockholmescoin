...
{
    QString ret = QString("sherlockholmescoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        if (ui->lnReqAmount->validate())
        {
            ...
            ret += QString("?amount=%1").arg(sherlockholmescoinUnits::format(sherlockholmescoinUnits::BTC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
