...
BOOST_AUTO_TEST_CASE(base58_keys_valid_parse)
{
    ...
    std::vector<unsigned char> result;
    CsherlockholmescoinSecret secret;
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if(isPrivkey)
        {
            ...
            // Note: CsherlockholmescoinSecret::SetString tests isValid, whereas CsherlockholmescoinAddress does not!
...
BOOST_AUTO_TEST_CASE(base58_keys_valid_gen)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        else
        {
            ...
            CsherlockholmescoinAddress addrOut;
            BOOST_CHECK_MESSAGE(boost::apply_visitor(CsherlockholmescoinAddressVisitor(&addrOut), dest), "encode dest: " + strTest);
            BOOST_CHECK_MESSAGE(addrOut.ToString() == exp_base58string, "mismatch: " + strTest);
        }
    }

    // Visiting a CNoDestination must fail
    CsherlockholmescoinAddress dummyAddr;
    CTxDestination nodest = CNoDestination();
    BOOST_CHECK(!boost::apply_visitor(CsherlockholmescoinAddressVisitor(&dummyAddr), nodest));
